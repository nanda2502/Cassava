...
std::vector<double> generateRawPayoffs(const std::vector<size_t>& shuffleSequence) {
    ...
    std::vector<double> payoffs(num_payoffs + 1, 0.0); // +1 for the root payoff
...
std::vector<double> generatePayoffs(const std::vector<size_t>& shuffleSequence) {
    std::vector<double> payoffs = generateRawPayoffs(shuffleSequence);
    return payoffs;
}
